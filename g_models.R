...
g_models <- function(an_data, subclass, match_weights, samp_weights, analyses) {
  ...
  runs <- runs[runs$method %in% c("pweight","tmle"),]
  ...
  # If there are weights, but the method is iptw, then the g model does not need to take these into account-- these are taken into account in the analysis phase. In this case, set all weights to 1. 
  ...
  if (is.na(subclass[1])) {
      ...
      for (i in 1:nrow(runs)) {
        ...
        if (temp_run$method == "pweight") { 
          temp_g <- est_g(an_data = an_data, match_weights = rep(1,nrow(an_data)), samp_weights = rep(1,nrow(an_data)), 
                          covs = temp_run$covs[[1]], # for iptw, don't need weights in estimation of propensity score; only in estimation phase
