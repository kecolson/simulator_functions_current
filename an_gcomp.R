...
# No need to use match_estimand as an input here, because the type of match doesn't influence the g-computation approach here
...
an_gcomp <- function(A, q, samp_weights, estimand, metric) {
  ...
  if (estimand == "ate") {
    ...
    EY1 <- weighted.mean(q[(n+1):(2*n)][tx.indices],   w = samp_weights[tx.indices])  
    EY0 <- weighted.mean(q[(2*n+1):(3*n)][tx.indices], w = samp_weights[tx.indices])  
  }
