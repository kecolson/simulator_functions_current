...
#        a single row of matches (as a list)
...
#                            match_covs = list(c("W1_0","W2_0","W3_0")), match_exact = F, match_distance = "pscore", match_method = "full", 
#                            match_nonexact_ratio = 1, match_replace = T, match_firstexact_covs = list(NA),
...
match <- function(sample, match_input) {
  ...
  if (match_input$match_exact == T) {
    ...
    return(list(an_data = an_data, subclass = rep(NA,nrow(an_data)), weights = an_data$weights, match_estimand = match_input$estimand))
  ...
  # Non-exact matching
    ...
    # Optimal
      ...
      if (sum(sample$A_1)/nrow(sample) >=0.5) {
        return(list(an_data = sample, subclass = rep(NA,nrow(sample)), weights = rep(NA,nrow(sample)), match_estimand = match_input$estimand))
      }
    ...
    # Subclassification  
      ...
      m.out <- matchit(as.formula(paste("A_1 ~",final.covs.formula)), data = sample, method = "subclass",
                       ...
                       reestimate = discard_reestimate)
      an_data <- match.data(m.out)
      return(list(an_data = an_data, subclass = an_data$subclass, weights = an_data$weights, match_estimand = match_input$estimand))
